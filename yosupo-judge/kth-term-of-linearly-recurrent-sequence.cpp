...
int bostan_mori(const vector<int>&a_, const vector<int>&c_, ll k){
	int d=a_.size();
	vector<int>c={1};
	for(int i=0;i<d;++i)
		c.push_back((Poly::mod-c_[i])%Poly::mod);
	Poly pa(a_),pc(c);
	Poly p=pa*pc;
	p.resize(d);
	int N=1;
	...
	nc.resize(N,0);
	temp.fft(nc,0);
	vector<int>curp=p.p,curc=c;
	vector<int>np(N),nu(N),nv(N);
	while(k){
		fill(np.begin(),np.end(),0);
		for(int i=0;i<d;++i)np[i]=curp[i];
		temp.fft(np,0);
		for(int i=0,j;i<N;++i){
			j=(i+(N>>1))&(N-1);
			nu[i]=(1ll*np[i]*nc[j])%Poly::mod;
			nv[i]=(1ll*nc[i]*nc[j])%Poly::mod;
		}
		temp.fft(nu,1);
		temp.fft(nv,1);
		fill(curp.begin(),curp.end(),0);
		for(int i=k&1;i<N and (i>>1)<d;i+=2)
			curp[i>>1]=nu[i];
		k>>=1;
		if(k){
			fill(curc.begin(),curc.end(),0);
			for(int i=0;i<N and (i>>1)<=d;i+=2)
				curc[i>>1]=nv[i];
			fill(nc.begin(),nc.end(),0);
			for(int i=0;i<=d;++i)nc[i]=curc[i];
			temp.fft(nc,0);
		}
	}
	return curp[0];
