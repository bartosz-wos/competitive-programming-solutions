...
vector<int>big_inv(const vector<int>&a){
	...
	if (n<=15){
		lll val=0;
		for(int i=n-1;i>=0;--i)val=(val<<4)+a[i];
		if(val==0)return {};
		lll num=1;
		for(int i=0;i<2*n;++i)num<<=4;
		lll q=num/val;
		vector<int>res;
		while(q){res.push_back((int)(q&15));q>>=4;}
		return res;
	}
	int k=(n+1)/2;
...
pair<vector<int>,vector<int>>big_div_mod(const vector<int>&a,const vector<int>&b){
	...
	if(m<=30){
		lll b_val=0;
		for(int i=m-1;i>=0;--i)b_val=b_val*16+b[i];
		vector<int>q(n,0);
		lll rem=0;
		for(int i=n-1;i>=0;--i){
			rem=rem*16+a[i];
		...
		vector<int>r;
		while(rem){r.push_back((int)(rem&15));rem>>=4;}
		return {q,r};
	}
	if(n>=(m<<1) and m>36){
		int split_pos=(n-m)/2;
...
char to_hex(int v){
        if(0<=v and v<=9)return (char)(v+48);
        return (char)(65+v-10);
}
