...
int power(int a, int b){
	return static_cast<int>(exp(a, b, Poly::mod));
}
...
void initb(){
	...
	inv[MAXN - 1] = power(fac[MAXN - 1], Poly::mod - 2);
	for(int i = MAXN - 2; i >= 0; --i)
		inv[i] = 1ll * inv[i + 1] * (i + 1) % Poly::mod;
}
...
int main(){
	...
	Poly B = P.poly_exp(n + 1);

	for(int i = 0; i <= n; ++i){
		int bell = 1ll * B[i] * fac[i] % Poly::mod;
		cout << bell << ' ';
	}
	cout << '\n';
}
